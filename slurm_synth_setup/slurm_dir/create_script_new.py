#added ntask and cpu-per-task logic according to deepspeed or nrmal data parallel cases vs DDP cases. 
import pandas as pd
import os

# == Configuration ==
input_csv = '/home/cc/slurm_dir/Mixed_P100_Workloads__16_jobs_.csv'
...
"""
    ...
    if model in llm_models:
        ...
        lines.append(f"deepspeed inference-test.py --name {model} --batch_size {int(row['batch_size'])}\n")
...
print(f"Generated {len(df)} SLURM scripts (*.sh).")
