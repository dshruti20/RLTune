#!/bin/bash
#SBATCH --job-name=job1_resnet152_1
#SBATCH --output=job1_resnet152_1.out
#SBATCH --gres=gpu:1
#SBATCH --ntasks=1
...
# 1) prepare per-job log dir
LOGDIR=~/job_logs/${SLURM_JOB_ID}
mkdir -p "$LOGDIR"

# 2) start CPU+memory sampler (records one line per second)
vmstat 1 > "$LOGDIR/cpu_mem.log" &
VMSTAT_PID=$!

# 3) start GPU sampler (records each GPU every second)
nvidia-smi \
...
python Training_LogCollection.py --arch resnet152 --epochs 2 --batch-size 64 --dataset mnist

# 5) tear down samplers
