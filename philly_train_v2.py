"""
Train Philly v2: same as Helios v2 (base + RL pipelines, tier1+lex allocation, optional MILP for RL).
Uses philly_env_base_v2 (PhillyEnvSkip, two pools 2-GPU/8-GPU, no mixing, merged tier1/tier2).
"""
import numpy as np
...
def ppo(workload_file, model_path, ac_kwargs=dict(), seed=0,
        ...
        cluster_name='philly', use_milp_allocation=False):
    ...
    env = PhillyEnvSkip(shuffle=shuffle, backfil=backfil, skip=skip, job_score_type=score_type,
    ...
    data_dir = logger_kwargs.get('data_dir', './data/logs/philly_v2')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Train Philly v2: base + RL pipelines, tier1+lex allocation')
    ...
    parser.add_argument('--exp_name', type=str, default='philly_v2')
    parser.add_argument('--pre_trained', type=int, default=0)
    parser.add_argument('--trained_model', type=str, default='./data/logs/philly_v2/philly_v2_s0')
    ...
    log_data_dir = os.path.join(current_dir, 'data', 'logs', 'philly_v2')
