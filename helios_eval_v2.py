"""
Eval script for v2 env and v2-trained models.
Uses helios_env_base_v2 (VC/cluster_name, 7-feature observation, reset_for_test, step_for_test_picktype).
Loads a policy from a v2 training run and compares RL vs baseline (e.g. SJF) on random 256-job batches.
"""
import time
import os
...
def run_policy(env, get_probs, get_out, nums, iters, score_type, sched_algo):
    ...
    os.makedirs("new_exp_figure/helios", exist_ok=True)
    plt.savefig("new_exp_figure/helios/test.png")
    plt.close()


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Eval v2-trained policy on v2 env')
    parser.add_argument('--rlmodel', type=str, default='data/logs/helios_v2/helios_v2_s0')
    ...
    parser.add_argument('--sched_algo', type=int, default=4, help='Baseline: 0=FCFS, 4=SJF, etc.')
