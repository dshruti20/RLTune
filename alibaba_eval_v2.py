"""
Eval script for Alibaba v2 env and v2-trained models.
Uses alibaba_env_base_v2 (AlibabaEnvSkip, 5 GPU-type pools, tier1/tier2 per pool same as Helios).
Loads a policy from a Alibaba v2 training run and compares RL vs baseline on random 256-job batches.
"""
import time
import os
...
def run_policy(env, get_probs, get_out, nums, iters, score_type, sched_algo):
    ...
    os.makedirs("new_exp_figure/alibaba", exist_ok=True)
    plt.savefig("new_exp_figure/alibaba/test.png")
    plt.close()


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Eval Alibaba v2-trained policy on Alibaba v2 env')
    parser.add_argument('--rlmodel', type=str, default='data/logs/alibaba_v2/alibaba_v2_s0')
